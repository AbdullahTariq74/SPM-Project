...
export default function VerifyEmailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#000814] flex items-center justify-center p-6 relative overflow-hidden">
      <button 
        onClick={toggleTheme} 
        className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-accent transition-all z-50 hover:scale-110"
      >
      ...
      <div className="w-full max-w-[420px] z-10">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-2xl shadow-2xl text-center">
          <div className="mb-8">
            ...
            <h1 className="text-2xl text-white font-black tracking-tighter uppercase">Verify Your Email</h1>
            <p className="text-slate-400 text-xs font-medium mt-2">We've sent a 6-digit code to <br/><span className="text-accent">{email}</span></p>
          </div>
            ...
            <div className="flex justify-between gap-2">
              {otp.map((data, index) => (
                <input
                  ...
                  type="text"
                  maxLength="1"
                  className="w-12 h-14 bg-white/5 border border-white/10 rounded-xl text-center text-xl font-black text-white focus:border-accent focus:ring-4 focus:ring-accent/5 outline-none transition-all"
                  value={data}
          ...
          <div className="mt-8 pt-8 border-t border-white/10">
            ...
            {resendTimer > 0 ? (
              <p className="text-white/40 text-xs font-bold">Resend available in {resendTimer}s</p>
            ) : (
              <button onClick={handleResend} className="text-accent font-black text-xs uppercase tracking-label hover:underline">Resend OTP Now</button>
            )}
          </div>
        </div>
      </div>
